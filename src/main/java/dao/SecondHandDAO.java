...
public class SecondHandDAO {
	...
	public int insertSecondHand(SecondHandDTO dto) throws Exception {
		String sql = "insert into secondHand values" + "(sh_seq.nextval, ?, ?, ?, default, default, ?, ?)";
	...
	public SecondHandDTO selectContents(int targetSeq) throws Exception {
		...
		try (Connection con = this.getConnection(); PreparedStatement pstat = con.prepareStatement(sql);) {
			...
			try (ResultSet rs = pstat.executeQuery();) {
				...
				return new SecondHandDTO(seq, title, contents, writer, write_date, view_count, lat, lng);
			}
		}
	}

	public List<SecondHandDTO> selectRecordByPage(int startRecord, int endRecord) throws Exception {
		...
		try (Connection con = this.getConnection(); PreparedStatement pstat = con.prepareStatement(sql);) {
			...
			try (ResultSet rs = pstat.executeQuery();) {
				...
				while (rs.next()) {
					...
					recordList.add(new SecondHandDTO(seq, title, contents, writer, write_date, view_count, lat, lng));
				}
				return recordList;
			}
		}
	...
	public List<SecondHandDTO> searchRecordByPage(int startRecord, int endRecord, String searchText, String option)
			throws Exception {
