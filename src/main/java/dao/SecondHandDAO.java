...
public class SecondHandDAO {
	...
	public int insertShFiles(Sh_FilesDTO dto) throws Exception{
		String sql = "insert into sh_files values(sh_files_seq.nextval, ?, ?, ?)";
		try(
	...
	public int getShSeqNextval() throws Exception{
		String sql = "select sh_seq.nextval from dual";
		try(
				Connection con = this.getConnection();
				PreparedStatement pstat = con.prepareStatement(sql);
	...
	public int insertSecondHandWithFiles(SecondHandDTO dto) throws Exception{
		String sql = "insert into secondHand values"
				+ "(?, ?, ?, ?, default, default, ?, ?)";
		try(
				Connection con = this.getConnection();
